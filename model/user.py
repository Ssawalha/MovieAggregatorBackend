...
class User(ORM):
    ...
    tablename = "users"
    fields = ["pk","username", "password_hash", "api_key", "first", "last"]
    ...
    def __repr__(self):
        msg = '<User pk:{pk}, username:{username}>'
    ...
    def set_password(self, password):
        self.values['password_hash'] = self.hash_password(password)
    
    def hash_password(self, password): #add saltedpw to db (same column), lookup salt in db to crack pword
        password = password.encode()
        salt = bc.gensalt()
        return bc.hashpw(password, salt)

    @classmethod
    def check_password(self, username, password): # should this be a classmethod?
        user = User()
        user_info = user.one_from_where_clause('WHERE username = ?', (username,))
        if user_info != None:
            password_hash = user_info.values['password_hash']
            password = password.encode()
            return bc.checkpw(password, password_hash)
        return False
    ...
    #     user_info = cls.one_from_where_clause("WHERE username = ?", (username,))

    #     check_password_bool = check_password(username, password)
